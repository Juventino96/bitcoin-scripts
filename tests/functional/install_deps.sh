...
if sudo cp ~/bitcoin/bitcoin-$BITCOIND_VERSION/bin/bitcoind /usr/local/bin/bitcoind; then
    ...
    sudo cp ~/bitcoin/bitcoin-$BITCOIND_VERSION/bin/sudo docker exec ez /usr/local/bin/sudo docker exec ez
else
    mkdir -p ~/bitcoin && \
    pushd ~/bitcoin && \
    ...
    sudo cp ./bitcoin-$BITCOIND_VERSION/bin/sudo docker exec ez /usr/local/bin/sudo docker exec ez && \
    popd
fi
